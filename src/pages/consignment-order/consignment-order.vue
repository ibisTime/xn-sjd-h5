    ...
    computed: {
      currentList() {
        if(this.categorysStatus[this.currentIndex]) {
          let _curListObj = this.conOrderList[this.categorysStatus[this.currentIndex].key];
          if (!_curListObj) {
            _curListObj = {
              start: 1,
              limit: 10,
    ...
    methods: {
      getInitData() {
        ...
        if (this.shouldGetData()) {
          this.first = false;
          // 清除缓存的订单列表数据
          this.setConOrderList({});
          this.getPageOrders();
        }
      },
      shouldGetData() {
        if (this.$route.path === '/consignment-order') {
          setTitle('寄售订单');
          return this.first;
        }
        return false;
      ...
      // 获取状态分类
      getCategorysStatus() {
        getDictList('group_order_status').then((res) => {
          res.map((item) => {
      ...
      getPageOrders() {
        ...
        if (this.currentList.hasMore) {
          getJishouOrderPage(this.currentList.start, this.currentList.limit, status, this.type).then((data) => {
            ...
            this.setConOrderList(_orderList);
          });
        }
      },
      ...mapMutations({
        'setConOrderList': SET_CON_ORDER_LIST,
        'setCurrentConOrder': SET_CURRENT_CON_ORDER
      }),
      ...mapActions([
        'editConOrderListByPay',
        'editConOrderListByCancel'
      ])
    },
    components: {
      MHeader,
